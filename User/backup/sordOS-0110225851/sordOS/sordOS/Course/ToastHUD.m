...

{
	...
	static ToastHUD *progressHUD;
	
	dispatch_once(&once, ^{ progressHUD = [[ToastHUD alloc] init]; });
...
//    NSURL *str = [[NSBundle mainBundle] URLForResource:@"hardenLibBundle.bundle" withExtension:nil];
//    NSBundle * bundle = [[NSBundle alloc] initWithURL:str];
    
    NSString *str = [[NSBundle mainBundle] pathForResource:@"sordOS" ofType:@"framework"];
    NSBundle *bundle = [NSBundle bundleWithPath:str];
	imageSuccess		= [UIImage imageNamed:@"progresshud-success.png" inBundle:bundle compatibleWithTraitCollection:nil];
	imageError			= [UIImage imageNamed:@"progresshud-error.png" inBundle:bundle compatibleWithTraitCollection:nil];
...

{
    ...
    if (@available(iOS 10.0, *)) {
        ...
        timer = [NSTimer scheduledTimerWithTimeInterval:delay target:self selector:@selector(ljwCustom) userInfo:nil repeats:NO];
    }
}
