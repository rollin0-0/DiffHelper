...
@property (strong, nonatomic) dispatch_source_t  dischargeAuthor;
...
- (void)setDischargeAuthor:(dispatch_source_t)dischargeAuthor {
    
    objc_setAssociatedObject(self, &dispatchSourceTimerFlag, dischargeAuthor, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (dispatch_source_t)dischargeAuthor {
...
- (void)startGlow {
    ...
    [self.layer.sublayers enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        if ([layer.name isEqualToString:GLOWVIEW_LAYER_FLAG]) {
            ...
            if (self.dischargeAuthor == nil) {
                
                self.dischargeAuthor = \
                dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER, 0, 0, dispatch_get_main_queue());
                dispatch_source_set_timer(self.dischargeAuthor, dispatch_time(DISPATCH_TIME_NOW, 0),
                                          NSEC_PER_SEC * (self.GCDTimerInterval == nil ? 1 : self.GCDTimerInterval.floatValue), 0);
                dispatch_source_set_event_handler(self.dischargeAuthor, ^{
