...
#import "BlowOverPitchBlackView.h"
#import "GiveOutLongPushHorizon.h"
#import "TWContentBarDirector.h"

@interface ViewController ()<MapHandlerLocatingAssign, UITableViewDelegate, FixEndureDataPointDelegate, WeatherViewDelegate, UIViewControllerTransitioningDelegate, FlushItFarseeingAgitateConsiderDesignate>

@property (nonatomic, strong) MapManager           *mapLoacation;
@property (nonatomic, strong) WeatherView          *weatherView;
@property (nonatomic, strong) BuzzOffBraveOutInformation       *getWeatherData;
@property (nonatomic, strong) BlowOverPitchBlackView        *fadeBlackView;
@property (nonatomic, strong) UpdateRegard         *upDatingView;
@property (nonatomic, strong) GiveOutLongPushHorizon  *failedView;
...
- (void)viewDidLoad {
    ...
    [self.weatherView materialBodyAspect];
    [self.view addSubview:self.weatherView];
    
    // 变黑
    self.fadeBlackView = [[BlowOverPitchBlackView alloc] initWithFrame:CGRectZero];
    [self.view addSubview:self.fadeBlackView];
    
    // loading
    self.upDatingView        = [[UpdateRegard alloc] initWithFrame:CGRectZero];
    ...
    // 获取网络请求
    self.getWeatherData          = [BuzzOffBraveOutInformation new];
    self.getWeatherData.delegate = self;
    
    // 加载失败后显示的view
    self.failedView          = [[GiveOutLongPushHorizon alloc] initWithFrame:self.view.bounds];
    self.failedView.delegate = self;
    [self.failedView materialBodyAspect];
...
- (void)pullUpEventWithData:(CurrentConditions *)condition {
    ...
    [GCDQueue executeInMainQueue:^{
        ...
        PrognosisAccountant *forecastCV    = [PrognosisAccountant new];
...
- (void)mapManager:(MapManager *)manager didFailed:(NSError *)error {
    ...
    [GCDQueue executeInMainQueue:^{
        ...
        [[TWContentBarDirector sharedInstance] registerContentWithEntitle:@"Failed to locate"
...
- (void)delayRunEvent:(id)object {
    ...
    [self.getWeatherData takeOffGetPlacementUpwindDataPoint];
}
...
- (void)showErrorInfo {
    ...
    [GCDQueue executeInMainQueue:^{
        ...
        [[TWContentBarDirector sharedInstance] registerContentWithEntitle:@"Network Unreachable"
