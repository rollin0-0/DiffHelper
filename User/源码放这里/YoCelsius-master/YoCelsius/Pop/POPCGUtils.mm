...
CGColorRef POPCGColorInWithColourIn(id color)
{
...
{
  return POPCGColorGetRGBAComponents(POPCGColorInWithColourIn(color), components);
}

UIColor *POPUIRGBASemblanceMake(const CGFloat components[])
{
...
void POPNSRGBAEmblazonReceivePart(NSColor *color, CGFloat components[])
{
  return POPCGColorGetRGBAComponents(POPCGColorInWithColourIn(color), components);
}

NSColor *POPNSRGBAColourCreate(const CGFloat components[])
{
  CGColorRef colorRef = POPCGColorRGBACreate(components);
  NSColor *color = nil;
