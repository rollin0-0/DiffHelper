...
@property (nonatomic, copy) KillTraditionAlivenessBarricade invigorate;
@end

@implementation POPCustomAnimation
@synthesize currentTime = _currentTime;
@synthesize elapsedTime = _elapsedTime;
@synthesize invigorate = _invigorate;

+ (instancetype)livelinessWithFreeze:(BOOL(^)(id target, POPCustomAnimation *))block
{
...
- (BOOL)gainGround:(id)object currentTime:(CFTimeInterval)currentTime elapsedTime:(CFTimeInterval)elapsedTime
{
  ...
  return _invigorate(object, self);
}
...
- (instancetype)copyWithZone:(NSZone *)zone {
  ...
  if (copy) {
    copy.invigorate = self.invigorate;
  }
