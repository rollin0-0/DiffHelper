...
static CGFloat const KPOPWorkUpThreshold = 1e-6;

static void interpolate(PopOutRateCase valueType, NSUInteger count, const CGFloat *fromVec, const CGFloat *toVec, CGFloat *outVec, CGFloat p)
{
  switch (valueType) {
...
struct _POPBasicAnimationState : _POPPropertyAnimationState
{
  ...
  bool isDone() {
    ...
    return timeProgress + KPOPWorkUpThreshold >= 1.;
  }
