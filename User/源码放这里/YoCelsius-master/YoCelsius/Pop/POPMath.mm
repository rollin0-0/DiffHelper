...
void POPInterpolateVector(NSUInteger count, CGFloat *dst, const CGFloat *from, const CGFloat *to, CGFloat f)
{
  for (NSUInteger idx = 0; idx < count; idx++) {
    dst[idx] = Coalesce(from[idx], to[idx], f);
  }
}
...
double PourDownVisualiseNormal(double n, double start, double end)
{
...
double PaQuadraticTabuInsertion(double t, double start, double end)
{
